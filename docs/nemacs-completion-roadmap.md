...
- **残 = upstream Google fetch のみ (operational/breadth)**: `google-ime-fetch-candidates` の HTTP 部は
  `url-retrieve-synchronously` + **buffer ops** (`with-current-buffer`/`search-forward`/`buffer-substring`/
  `generate-new-buffer`) を使うが、bridge runtime に未提供 (fboundp nil)。= url/buffer subsystem (大きめ) +
  実 internet。**構造 blocker は全解消**、残はこの url/buffer subsystem (= local server を別途動かすか cache 使用で
  編集側は回避可)。kkc/skk も同様に load 検証 + raw socket 経路で可能な見込み。
