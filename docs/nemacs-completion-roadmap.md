...
- **残 breadth (fetch-candidates の完全動作に)**: (1) `replace-regexp-in-string` (JSON cleanup、fboundp nil
  → abort。google-ime は "\n"/",]" の literal 置換に使用 = 簡易 polyfill 可)、(2) `url-hexify-string`
  (query URL encode、url-util は source-v1 で abort → 小 polyfill 可)、(3) `url-retrieve-synchronously`
  (実 HTTP to Google = internet + HTTP client、operational)。(1)(2) は小 polyfill、(3) のみ外部依存。
- **構造 blocker は全て解消**。残 = 上記 stdlib 関数 polyfill (小) + 実 HTTP/server (operational)。
