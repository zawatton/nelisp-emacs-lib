#+TITLE: nemacs「Emacs 置き換え完遂」ロードマップ (2026-06-13 時点)
#+DATE: 2026-06-13
...
本書 (2026-06-13/14) の主要 blocker は、その後の runtime 前進で *軒並み解消済*。
検証手順・証拠は =docs/design/15-staged-execution-plan-small-medium-large.org=
§3.2-3.4 を参照。要約:

- ✅ *buffer-based file reading (本書「統一 blocker」)*: 解消。skk は
  =nemacs-runtime-cdb= の独自 =cdb--read= で日本語変換 E2E 5/5 pass
  (みらい→未来、はしる+SPC→走る)。GUI bridge 汎用 =insert-file-contents= の
  BEG/END range も修正済 (Doc 15 B2)。
- ✅ *runtime 生 defun は登録されない (本書「最重要 next frontier」)*: 解消。
  =--load= / bridge / eval 全経路で defun/defmacro/cl-defstruct/when-let が
  load 時展開。実 dash.el (148KB) 動作 (=-map= 等 callable) (Doc 15 B1)。
- ✅ *process 層*: make-process / start-process / call-process / comint-mode /
  process-send-string / accept-process-output 既に揃う (Doc 11 §3.8 も stale、
  Doc 15 B3)。
- ◑ *残 = builtin breadth*: package が呼ぶ未実装 builtin (save-match-data /
  replace-match string 形 等) を実 package 駆動で逐次補完する段階 (Doc 15 B4)。
  macroexpand-all 等 long-tail 継続。

#+BEGIN_COMMENT
以下は 2026-06-13/14 時点の本文。歴史記録として保持。上記訂正で上書きされた
箇所には行頭 # の inline 訂正ノートを付した。
#+END_COMMENT
...
# 【2026-06-20 訂正 (Doc 15 §3.2)】解消済。skk は nemacs-runtime-cdb の独自
# cdb--read で日本語変換 E2E 5/5 pass (みらい→未来、はしる+SPC→走る)。GUI bridge
# 汎用 insert-file-contents の BEG/END range も修正済。以下は当時の blocker 記録。
...
- *✅ float-time SHIPPED (hand-asm extern, fe221c3c)*: =(float-time)==1781370652.5
  ...
  # 【2026-06-20 訂正 (Doc 15 §3.3)】解消済。=--load= / bridge / eval の全経路で
  # defun / defmacro / cl-defstruct / when-let が load 時展開・登録される。実
  # dash.el (148KB) が動作 (=-map= 等 callable)。以下は当時の制約記録。
...
- *残 (P1 続き)*: (b') =mod= の float (稀)。current-time (float-time と同様 hand-asm でいける)。
  # 【2026-06-20 訂正 (Doc 15 §3.3/§3.4)】prelude / runtime macro 展開は解決済。
  # 残は package が呼ぶ未実装 builtin の breadth (save-match-data / replace-match
  # string 形 等を逐次補完済、long-tail 継続)。以下は当時の診断記録。
