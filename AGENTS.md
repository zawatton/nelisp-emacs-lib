...
This repository is in a library-first phase.  The primary goal is to make
the pure-Elisp replacements for Emacs C-core behavior reusable by other
projects, with `nemacs` and GUI/TUI frontends as consumers.
...
Reusable behavior belongs in library modules, not in application or
frontend glue.

Allowed ownership:

- foundation primitives: `emacs-fns`, `emacs-list`, `emacs-string`,
  `emacs-symbol`, `emacs-numeric`, `emacs-pcase`, `emacs-cl-macros`, etc.
- buffer/edit semantics: `nelisp-emacs-compat`, `emacs-buffer`,
  line/search/edit/undo modules.
- editor semantics: window, frame, keymap, command-loop, minibuffer,
  modes, faces, help/info.
- adapters: file/process/network/sqlite/standalone/TUI/GUI transport.
- apps: `nemacs-loadup`, `emacs-init`, `nemacs-main`, launchers.

Do not put command semantics, buffer mutation, keymap lookup, minibuffer
completion, undo, kill-ring, file command behavior, or window layout logic
inside GUI/TUI/app glue when a shared module can own it.

## API Policy

- Prefer prefixed public APIs (`emacs-buffer-*`, `emacs-window-*`,
  `emacs-keymap-*`, `nelisp-ec-*`).
- Install unprefixed Emacs-compatible names only through explicit shim or
  builtins modules.
- Treat prefixed helpers defined in `emacs-*-builtins.el` as shim helper
  surface by default, not stable external API, unless the facade stable API
  manifest explicitly promotes them.
- Treat prefixed guard helpers such as `*-check-live` as internal
  validation surface unless an external consumer use case justifies facade
  stable API promotion.
- Treat `--` functions as private.  Cross-module use of private helpers
  should be promoted deliberately or removed.
- Use `make nemacs-public-api-inventory` to inspect the current
  package-group API surface before declaring a new reusable API stable.
- `make nemacs-library-gate` must keep public API inventory `UNOWNED`
  rows at zero; update Doc 18 ownership for new reusable files.
- `make nemacs-ownership-coverage` must keep file-level `unowned` and
  `stale` rows at zero for all `src/` and `gui/` Elisp files.
- Reusable `*-features` manifests must stay duplicate-free, resolvable on
  `load-path`, and free of app/bootstrap or concrete TUI/GUI features.
- Keep `nelisp-emacs-library-package-manifest` aligned with facade and
  group loader manifests when changing reusable package membership.
...
- Run `make nemacs-library-package-verify` when descriptor, guide, package
  ...
  stay zero.
- Prefer the facade query helpers (`nelisp-emacs-library-package-names`,
  `nelisp-emacs-library-package`,
  `nelisp-emacs-library-package-features`, and
  `nelisp-emacs-library-package-lazy-features`) over open-coding manifest
...
- Keep `make nemacs-library-consumer-smoke` green when changing facade
  membership; external consumers must be able to require `nelisp-emacs`
  from `emacs -Q -L src` without loading app/bootstrap or frontends.
...
- Host Emacs compatibility must not silently override host C primitives
  unless the module is explicitly a compatibility shim and is gated.

## nelisp-gui Policy

`nelisp-gui` may be reintegrated under this tree while APIs mature.  During
that phase, GUI is a consumer and validation target, not the owner of Emacs
semantics.

GUI may own protocol, raw input decoding, transport, window-system state,
and painting.  Shared runtime modules own command behavior and editor state.

## Development Flow

Use anvil defs/call-graph tools before broad refactors:

- search definitions first,
- inspect references/callers,
- use architecture/clusters for subsystem boundaries,
- avoid reading large files blindly when indexed structure is enough.

For verification, start with focused ERTs and `make test-fast`.  For
library-surface work, run `make nemacs-library-gate`; for facade-only
changes, `make nemacs-library-contract` and
`make nemacs-library-consumer-smoke` are the focused preflights.
For group loader changes, run `make nemacs-library-package-smoke`.  Also
inspect `make nemacs-public-api-inventory` and
`make nemacs-library-package-manifest`; for dependency shape changes,
also run `make nemacs-library-package-deps` and review its migration queue.
For package extraction shape changes, run
...
`make nemacs-library-package-verify`.  If a change touches
bootstrap/runtime images, run `make verify-production-runtime-path`.  If it
touches TUI or bridge behavior, follow `docs/design/12-development-gates.org`.

## Review Checklist

Before finishing a change, answer:

- Which library group owns this behavior?
- Can another project reuse it without loading `nemacs-main`?
- Is the API prefixed, or is it intentionally an Emacs compatibility shim?
- Did this add app/frontend-local semantics that should be shared?
- Are fallback/stub paths visible and tracked?
- Is there a focused test near the changed module?
